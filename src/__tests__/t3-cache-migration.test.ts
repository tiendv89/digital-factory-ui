...
import type { TaskDetail, FeatureDetail } from "../services/workflow-backend/types";
...
import { useFeatureDetail, useFeatureTask } from "../features/board/hooks/useFeatureDetail";
...
function makeTaskDetail(overrides: Partial<TaskDetail> = {}): TaskDetail {
  return {
    ...
    depends_on: [],
    execution: { actor_type: "agent", last_updated_by: "bot@x.com", last_updated_at: "2026-01-01T00:00:00Z" },
    pr_refs: [],
    ...overrides,
  };
}
...
describe("useWorkspaceTask — query key and cache behavior", () => {
  ...
  it("returns cached data on second render without a new fetch", async () => {
    ...
    const { result: r1 } = renderHook(() => useWorkspaceTask("ws-1", "task-uuid-1"), { wrapper });
    ...
    const { result: r2 } = renderHook(() => useWorkspaceTask("ws-1", "task-uuid-1"), { wrapper });
  ...
  it("does not fetch when workspaceId is null", async () => {
    const { result } = renderHook(
      () => useWorkspaceTask(null, "task-uuid-1"),
      { wrapper: makeWrapper(client) },
    );
  ...
  it("exposes error from a failed fetch", async () => {
    const apiErr = { code: "NOT_FOUND", message: "Task not found", retryable: false };
    vi.mocked(getWorkspaceTask).mockRejectedValueOnce(apiErr);

    const { result } = renderHook(
      () => useWorkspaceTask("ws-1", "bad-id"),
      { wrapper: makeWrapper(client) },
    );
  ...
  it("uses independent cache entries for different workspaces", async () => {
    ...
    const wrapper = makeWrapper(client);
    const { result: r1 } = renderHook(() => useWorkspaceTask("ws-1", "task-uuid-1"), { wrapper });
    const { result: r2 } = renderHook(() => useWorkspaceTask("ws-2", "task-uuid-1"), { wrapper });
...
describe("useFeatureDetail — query key and cache behavior", () => {
  ...
  it("returns cached data on second render without a new fetch", async () => {
    ...
    const wrapper = makeWrapper(client);
    const { result: r1 } = renderHook(() => useFeatureDetail("ws-1", "feat-uuid-1"), { wrapper });
    await waitFor(() => expect(r1.current.loading).toBe(false));
    expect(getFeature).toHaveBeenCalledTimes(1);

    const { result: r2 } = renderHook(() => useFeatureDetail("ws-1", "feat-uuid-1"), { wrapper });
    await waitFor(() => expect(r2.current.loading).toBe(false));
    expect(getFeature).toHaveBeenCalledTimes(1);
  ...
  it("does not fetch when workspaceId is null", async () => {
    const { result } = renderHook(
      () => useFeatureDetail(null, "feat-uuid-1"),
      { wrapper: makeWrapper(client) },
    );
    await act(async () => {});
  ...
  it("exposes error from a failed fetch", async () => {
    const apiErr = { code: "NOT_FOUND", message: "Feature not found", retryable: false };
  ...
  it("uses independent cache entries for different workspaces", async () => {
    const feat1 = makeFeatureDetail({ workspace_id: "ws-1", title: "WS1 Feature" });
    const feat2 = makeFeatureDetail({ workspace_id: "ws-2", title: "WS2 Feature" });
    ...
    const wrapper = makeWrapper(client);
    const { result: r1 } = renderHook(() => useFeatureDetail("ws-1", "feat-uuid-1"), { wrapper });
    const { result: r2 } = renderHook(() => useFeatureDetail("ws-2", "feat-uuid-1"), { wrapper });
...
describe("useFeatureTask — query key and cache behavior", () => {
  ...
  it("fetches feature-task data using workspace-scoped query key", async () => {
    ...
    expect(result.current.task).not.toBeNull();
    expect(result.current.error).toBeNull();
    expect(getFeatureTask).toHaveBeenCalledWith("ws-1", "feat-uuid-1", "task-uuid-1");
  });
  ...
  it("exposes error from a failed fetch", async () => {
    const apiErr = { code: "NOT_FOUND", message: "Feature task not found", retryable: false };
  ...
  it("uses independent cache entries for different workspaces", async () => {
    const task1 = makeTaskDetail({ workspace_id: "ws-1", title: "WS1 FeatureTask" });
    const task2 = makeTaskDetail({ workspace_id: "ws-2", title: "WS2 FeatureTask" });
  ...
  it("query key matches workspaceKeys.featureTask shape", () => {
    ...
    expect(key).toEqual(["workspace", "ws-abc", "feature", "my-feature", "task", "T7"]);
  });
...
describe("Cache pre-seeding — revisit without loading wait", () => {
  ...
  it("useFeatureTask returns pre-seeded data immediately (no loading)", () => {
    ...
    client.setQueryData(workspaceKeys.featureTask("ws-1", "feat-uuid-1", "task-uuid-1"), task);
