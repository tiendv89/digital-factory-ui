/**
 * T4 — Feature Task Docs markdown panel tests
 *
 * Covers:
 ...
 *   - Empty fallback: fallback message presence in component tree
...
describe("FeatureTasksPanel — Task Docs subview states", () => {
  ...
  it("renders empty fallback text for Task Docs (not legacy 'tasks.md')", () => {
    mockUseDocumentContent.mockReturnValue(
    ...
    expect(html).toContain("Task Docs");
    expect(html).not.toContain("No tasks.md");
  });

  it("renders 'Open in GitHub' link when doc URL is available with no content", () => {
    mockUseDocumentContent.mockReturnValue(
      defaultDocContentResult({ content: null, loading: false }),
    );
