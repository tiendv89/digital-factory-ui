...
import type { Member, Invitation, InviteRequest } from "@/services/user-service";

const adminKeys = {
  members: (workspaceId: string) =>
    ["admin", "workspace", workspaceId, "members"] as const,
  invitations: (workspaceId: string) =>
    ["admin", "workspace", workspaceId, "invitations"] as const,
};
...
export function useWorkspaceInvitations(workspaceId: string | null) {
  const { data, isLoading, error, refetch } = useQuery<Invitation[], Error>({
    queryKey: workspaceId
      ? adminKeys.invitations(workspaceId)
      : ["admin-invitations-disabled"],
    queryFn: () =>
      fetchWorkspaceInvitations(workspaceId!).then((r) => r.invitations),
    enabled: Boolean(workspaceId),
  });
  return {
    invitations: data ?? [],
