...
function StatusSpinner({ className }: { className?: string }) {
  return (
    <svg
      className={["status-spinner", className].filter(Boolean).join(" ")}
      width="12"
      height="12"
      viewBox="0 0 14 14"
...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  const baseClass =
    "inline-flex items-center gap-1.5 border border-border px-2 py-0.5 text-xs font-semibold uppercase tracking-wide " +
    statusStyle.bg +
    " " +
    statusStyle.text;
