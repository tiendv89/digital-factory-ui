...
export function findStatusLogEntry(log: LogEntry[] | undefined, status: string): LogEntry | null {
...
export function getTaskLastUpdatedAt(task: Pick<ParsedTask, "execution" | "log" | "updatedAt">): string | null {
...
export function computeLastUpdatedLabel(task: Pick<ParsedTask, "execution" | "log" | "updatedAt">, now: Date = new Date()): string | null {
...
export function computeStatusAge(task: Pick<ParsedTask, "status" | "log" | "execution">, now: Date = new Date()): string {
...
export function getElapsedSinceStatus(task: Pick<ParsedTask, "status" | "log">, now: Date = new Date()): string {
  const entry = findStatusLogEntry(task.log, task.status);
  if (!entry) return "—";
...
function latestValidIsoTimestamp(current: string | null, candidate: string | undefined): string | null {
