import { FEATURE_MODE_STATUSES, type FeatureStatus, TASK_MODE_STATUSES, type TaskStatus } from "./status";

export function getAllStatusFilterKeys(): TaskStatus[] {
  return [...TASK_MODE_STATUSES];
}

export function isAllStatusFilterSelected(selectedStatuses: readonly string[]): boolean {
  const selected = new Set(selectedStatuses);
  return TASK_MODE_STATUSES.every((status) => selected.has(status));
}

export function toggleAllStatusFilter(selectedStatuses: readonly string[]): TaskStatus[] {
  return isAllStatusFilterSelected(selectedStatuses) ? [] : getAllStatusFilterKeys();
}

export function toggleStatusFilter(selectedStatuses: readonly string[], status: string): string[] {
