...
export function CreateWorkspaceModal({ onClose, onSuccess }: CreateWorkspaceModalProps) {
  ...
  const handleCreate = useCallback(
    ...
    [name, slug, mutate, addWorkspace, onSuccess, onClose],
  );
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={mode === "create" ? "Create workspace" : "Import workspace"}
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-[13px] border border-border bg-surface shadow-[0_8px_20px_rgba(0,0,0,0.5)]">
