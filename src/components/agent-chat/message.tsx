...
export function Message({ message }: { message: HermesMessage }) {
  ...
  return (
    <div data-message data-role={message.role} className={`flex w-full ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${isUser ? "bg-primary text-white" : "bg-surface-secondary text-text-primary"}`}>
