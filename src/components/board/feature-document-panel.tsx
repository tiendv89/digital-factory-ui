...
import { AlertCircle, Edit2, ExternalLink, Eye, RefreshCw, X } from "lucide-react";
...
export function FeatureDocumentPanel({ feature, documentType }: FeatureDocumentPanelProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setStaleConflict(false);
    try {
      await saveDocument(feature.feature_id, documentType as "product_spec" | "technical_design", editText, docContent?.sha ?? null);
      setMode("view");
      setIsDirty(false);
